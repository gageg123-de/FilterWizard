...
Two production pilot pages exist: [`../filter-sizes/20x25x1.html`](../filter-sizes/20x25x1.html) and [`../filter-sizes/16x25x1.html`](../filter-sizes/16x25x1.html). The directory is file-routed and has no index page, generator, data source or bulk system. The pilot set is an implemented pattern, not authorization to create more pages.
...
- URL: `/filter-sizes/{normalized-size}.html`; only `20x25x1.html` and `16x25x1.html` are implemented.
...
Minimum launch gate for scaling: both pilots are indexed and useful, no major cannibalization or wrong-size behavior is observed, retailer queries preserve warnings/disclosure, a rollback/removal plan exists, and a human owns data quality. No threshold values are currently approved; do not invent them. Code-emitted events do not prove these gates; external dashboard evidence is still required.
