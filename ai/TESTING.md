...
For every filter-size manifest or page change, run `node tools/filter-size-pages.mjs validate`. A passing result is required in addition to browser checks; it validates manifest/page parity, metadata, canonicals, schema presence, sitemap membership, affiliate query/tag construction, tracking hooks, and near-duplicate content. Also run `node tools/internal-link-audit.mjs --output internal-link-report.json`, review the report, and do not commit the generated report.

For another size page, compare metadata, headings, paragraphs, FAQs and confusion guidance against all existing pilots. Shared structure and required disclosures may repeat; substantive exact-copy overlap should be limited and explained.
