...
- Filter-size Page Guardrail: before drafting a size URL, verify that the nominal size is a marketed residential size with defensible manufacturer/retailer evidence and that the page can provide distinct homeowner utility beyond replacing dimension strings. Mathematical possibility or one retailer-search result is insufficient. Reject thin or interchangeable candidates, never invent universal actual dimensions, use the structured manifest/generator, and run `node tools/filter-size-pages.mjs validate` per [`FILTER_SIZE_PAGES.md`](ai/FILTER_SIZE_PAGES.md).
...
- Internal-Linking Guardrail: integrate every new article, size guide, or content hub into the site's topical architecture at publication time. Identify its primary intent owner and cluster, include useful outgoing contextual links, add at least one justified incoming link from an existing page when appropriate, choose Related Articles intentionally, and evaluate the Filter Finder path. Links must help the user and must not blur intent ownership, manufacture reciprocity, or create cannibalization. Follow [`INTERNAL_LINKING.md`](ai/INTERNAL_LINKING.md) and run the internal-link audit before completion.
...
- Internal architecture: [`INTERNAL_LINKING.md`](ai/INTERNAL_LINKING.md)
